...
```
```{r hp}
# Filter for high production countries and include "World"
...
# Assuming top_production_countries and least_production_countries are defined
...
average_ratio <- ratio_data %>%
  ...
  arrange(desc(Average_Ratio))  # Arrange in descending order
...
```{r  sum_dom}
# Summarizing the data to get average domestic supply and production by Country and Year
...
ggplot(domestic_supply_summary, aes(x = Average_Production, y = Average_Domestic_Supply)) +
  geom_point(aes(color = Country)) +
  geom_smooth(method = "lm", se = FALSE) +
  theme_minimal() +
  labs(title = "Average Domestic Supply vs. Production Over the Years",
       x = "Average Production",
       y = "Average Domestic Supply")
...
# Summarizing average domestic supply and average production by country
comparison_summary <- domestic_supply_summary %>%
  group_by(Country) %>%
  summarise(
    Average_Domestic_Supply = mean(Average_Domestic_Supply, na.rm = TRUE),
    Average_Production = mean(Average_Production, na.rm = TRUE),
    .groups = 'drop'
  )

# Displaying the summary for comparison
print(comparison_summary)


# Creating a new data frame for plotting
comparison_plot_data <- data.frame(
  Country = rep(comparison_summary$Country, each = 2),
  Metric = rep(c("Average_Domestic_Supply", "Average_Production"), times = nrow(comparison_summary)),
  Value = c(comparison_summary$Average_Domestic_Supply, comparison_summary$Average_Production)
)

# Displaying the combined data frame
print(comparison_plot_data)


# Plotting the comparison
ggplot(comparison_plot_data, aes(x = Country, y = Value, fill = Metric)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Comparison of Average Domestic Supply and Production",
       x = "Country",
       y = "Average Value") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
```{r plot}
# Summarizing total production and total food waste by country
...
ggplot(comparison_data_melted, aes(x = Country, y = Value, fill = Metric)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Comparative Analysis of Production and Food Waste by Country",
       x = "Country",
       y = "Total Value") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
