...
#checking the dimension of filtered data
dim(df_filtered)
...
#checking the dimension of removing countries will all values as 0 or NA
dim(df_cleaned)
...
average_growth_rate <- mean(null_data$GrowthRate, na.rm = TRUE)
...
final_df <- subset(final_df, Country != "Micronesia (FAO)")
...
for (col in c("Population","Land_Use")) {  # Exclude the Production column
  p <- ggplot(final_df, aes_string(x = "Production", y = col, color="Country")) +
    geom_point() +
    labs(title = paste("Scatter Plot of Production vs", col),
         x = "Production",
         y = col) +
    theme_minimal() +
...
#scatter plot with production vs other parameters colured by year
# Create a list to store plots
plots <- list()
