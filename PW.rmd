...
```
The graph shows the rice production trends of the top and least producing countries from 1980 to 2020.

China and India are the top producers, with China consistently exceeding 150 million tonnes since 1985.Both countries peaked around 200 million tonnes in 2015.

Vietnam, Thailand, Myanmar, Indonesia, Bangladesh, and 6 other unnamed countries are among the least 10 producers in 2020.

Vietnam saw the largest increase in rice production, reaching nearly 45 million tonnes by 2020.

Indonesia, despite starting with higher production, has lagged behind, reaching around 35 million tonnes.
...
```
```{r supply}
average_supply_chain_waste <- filtered_data %>%
  group_by(Country) %>%
  summarise(Average_Supply_Chain_Waste = mean(Supply_Chain_Waste_t, na.rm = TRUE), .groups = 'drop') %>%
  arrange(desc(Average_Supply_Chain_Waste))  # Arrange in descending order

# Display the results
print(average_supply_chain_waste)
```
...
descriptive_stats
```
We Calculated Average Waste to Production Ratio with the formula Total Waste/ Total Production
```{r ratio}
...
```
An ANOVA test was performed to compare average domestic supply across countries.
...
```{r chi}
average_supply_chain_waste <- average_supply_chain_waste %>%
  mutate(Supply_Chain_Category = case_when(
    Average_Supply_Chain_Waste < 1500000 ~ "Low",
    Average_Supply_Chain_Waste < 3000000 ~ "Medium",
    TRUE ~ "High"
  ))

total_food_waste <- total_food_waste %>%
  mutate(Food_Waste_Category = case_when(
    Total_Food_Waste < 500000000 ~ "Low",
    Total_Food_Waste < 1500000000 ~ "Medium",
    TRUE ~ "High"
  ))


merged_data <- merge(average_supply_chain_waste, total_food_waste, by = "Country")
merged_data
```

```{r chi-sq}
#Create a contingency table
contingency_table <- table(merged_data$Supply_Chain_Category, merged_data$Food_Waste_Category)
